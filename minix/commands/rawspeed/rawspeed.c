...
int main(int argc, char **argv)
{
	...
	if (randlimit != 0 && tenthsec >= 5) {
		...
		"%ld seeks / %ld.%ld s = %ld seeks/s = %ld.%d ms/seek\n",
			(long)nseeks, tenthsec / 10, tenthsec % 10,
			(long)((nseeks * 10 + tenthsec/2) / tenthsec),
			(long)(tenthms / 10), (int)(tenthms % 10));
