...
/*
 * Intel architecture performance counters watchdog (amd64 port).
 * MSR addresses and logic are identical to i386; only the lockup
 * print uses 64-bit format strings and the start function is renamed.
 */
...
int arch_watchdog_init(void)
{
	...
	if (cpu_info[cpu].vendor == CPU_VENDOR_INTEL) {
		 ...
		 * architectural performance counters. Some Intel CPUs don't have
		 * this feature.
		 */
