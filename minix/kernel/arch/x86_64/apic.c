 ...
 *
 * Adapted from i386 version for x86_64 (LP64):
 *   - ioapic_read/write base addresses are vir_bytes (64-bit pointer)
 *   - lapic_addr_vaddr is vir_bytes
 *   - All (u32_t)addr casts replaced with (vir_bytes)
 *   - APIC register values remain u32_t (hardware registers are 32-bit)
 */
#include <assert.h>
...
int ioapic_enabled;
vir_bytes lapic_addr_vaddr;	/* LP64: pointer-sized, not u32_t */
...
/* LP64: ioa_base is a virtual address (pointer-sized), not u32_t */
...
static void ioapic_redirt_entry_write(void * ioapic_addr,
					...
					u32_t lo)
{
	/* LP64: cast void* to vir_bytes for ioapic_write */
	ioapic_write((vir_bytes)ioapic_addr,
		(u8_t) (IOAPIC_REDIR_TABLE + entry * 2 + 1), hi);
	ioapic_write((vir_bytes)ioapic_addr,
		(u8_t) (IOAPIC_REDIR_TABLE + entry * 2), lo);
}
...
static void ioapic_disable(struct io_apic * ioapic)
{
	...
	for (p = 0; p < io_apic->pins; p++) {
		u32_t low_32, hi_32;
		/* LP64: io_apic->addr is vir_bytes, no cast needed */
