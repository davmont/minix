...
/*
 * 8-byte "normal" segment descriptor.
 * In 64-bit mode the limit/base fields of code and data descriptors are
 * ignored by the CPU, but the access byte and the attribute nibble still
 * matter (DPL, present, L bit for code segments).
 */
struct segdesc_s {
    u16_t limit_low;
    u16_t base_low;
    u8_t  base_middle;
    u8_t  access;       /* present, DPL, type */
    u8_t  granularity;  /* limit_high[3:0], AVL, L, D/B, G */
    u8_t  base_high;
