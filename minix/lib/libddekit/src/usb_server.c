...
(struct ddekit_usb_dev * dev, unsigned int interfaces) {
	...
	if (dudev == NULL) {
		ddekit_printf("ERROR: devman_usb_device_new failed\n");
		return;
	}
	...
	if (res != 0) {
		ddekit_printf("ERROR: devman_usb_device_add failed\n");
	}
}
...
static void device_disconnect_callback(struct ddekit_usb_dev * dev)
{
	...
	if (dudev == NULL) {
		ddekit_printf("ERROR: ddekit_usb_dev_get_data(dev) failed, dudev is NULL!\n");
		return;
	}
