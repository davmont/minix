...
static u8_t my_inb(u32_t port) {
	...
	value = *(volatile u8_t *)(uintptr_t)(port);
#else
...
static u16_t my_inw(u32_t port) {
	...
	int r;
#ifdef DMA_BASE_IOMAP
	value = *(volatile u16_t *)(uintptr_t)(port);
#else
...
static u32_t my_inl(u32_t port) {
	...
	int r;
#ifdef DMA_BASE_IOMAP
	value = *(volatile u32_t *)(uintptr_t)(port);
#else
...
static void my_outb(u32_t port, u32_t value) {
	int r;
#ifdef DMA_BASE_IOMAP
	*(volatile u8_t *)(uintptr_t)(port) = value;
#else
...
static void my_outw(u32_t port, u32_t value) {
	int r;
#ifdef DMA_BASE_IOMAP
	*(volatile u16_t *)(uintptr_t)(port) = value;
#else
...
static void my_outl(u32_t port, u32_t value) {
	int r;
#ifdef DMA_BASE_IOMAP
	*(volatile u32_t *)(uintptr_t)(port) = value;
#else
