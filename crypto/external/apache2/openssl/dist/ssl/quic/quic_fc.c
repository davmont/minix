...
/*
 * MINIX: pre-10 NetBSD <stdint.h> defines uint64_t as a macro, so using it
 * as the safe-math name tag pastes the wrong identifier; use "qu64" like
 * ring_buf.h uses "u64".
 */
OSSL_SAFE_MATH_UNSIGNED(qu64, uint64_t)
...
static int rxfc_cwm_bump_desired(QUIC_RXFC *rxfc)
{
    ...
    uint64_t threshold
        = safe_muldiv_qu64(rxfc->cur_window_size,
