...
#endif
#if defined(__minix)
/* MINIX: headers define MSG_WAITFORONE but libc has no {send,recv}mmsg */
#undef NO_RECVMMSG
#define NO_RECVMMSG
#endif
#if !defined(M_METHOD)
...
/* MINIX: struct in_pktinfo has no ipi_spec_dst (pre-10 NetBSD netinet) */
#if !defined(OPENSSL_SYS_WINDOWS) && !defined(OPENSSL_SYS_CYGWIN) && !defined(__FreeBSD__) && !defined(__QNX__) && !defined(__minix)
