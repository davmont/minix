...
out:
	...
	if (tag_files != NULL) {
		...
		tag_write();
#if defined(__minix)
		/* MINIX libc has getpgrp() but no getpgid().  Approximate
		 * the parent-pgid lookup with our own pgrp; the mismatch
		 * path in tag_unlink() handles the rest. */
		man_pgid = getpgrp();
		tag_files->tcpgid = man_pgid;
#else
		man_pgid = getpgid(0);
...
woptions(struct curparse *curp, char *arg)
{
	...
	while (*arg) {
		o = arg;
		switch (getsubopt(&arg, __UNCONST(toks), &v)) {
		case 0:
			curp->wstop = 1;
			break;
		case 1:
