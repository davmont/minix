/* $NetBSD: t_stpcpy.c,v 1.1 2011/07/07 08:59:33 jruoho Exp $ */
...
ATF_TC(stpcpy_basic);
ATF_TC_HEAD(stpcpy_basic, tc)
{
        atf_tc_set_md_var(tc, "descr", "Test stpcpy(3) results");
}

ATF_TC_BODY(stpcpy_basic, tc)
{
	/* try to trick the compiler */
	char * (*f)(char *, const char *s) = stpcpy;
	...
	for (a0 = 0; a0 < sizeof(long); ++a0) {
		for (a1 = 0; a1 < sizeof(long); ++a1) {
			for (t = 0; t < (sizeof(tab) / sizeof(tab[0])); ++t) {
				...
				/*
				 * verify stpcpy returns address of
				 * terminating null byte
				 */
				if (ret != &buf0[a0] + tab[t].len) {
					fprintf(stderr, "a0 %d, a1 %d, t %d\n",
					    a0, a1, t);
					atf_tc_fail("stpcpy did not return "
					    "terminating null byte address");
				}
