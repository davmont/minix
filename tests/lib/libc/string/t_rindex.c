/* $NetBSD: t_rindex.c,v 1.1 2011/07/07 08:59:33 jruoho Exp $ */
...
ATF_TC(rindex_basic);
ATF_TC_HEAD(rindex_basic, tc)
{
        atf_tc_set_md_var(tc, "descr", "Test rindex(3) results");
}

ATF_TC_BODY(rindex_basic, tc)
{
	/* try to trick the compiler */
	char * (*f)(const char *, int) = rindex;
		...
		for (t = 0; t < (sizeof(tab) / sizeof(tab[0])); ++t) {
			...
			if (tab[t].match == 0) {
				if (off != 0) {
					...
					atf_tc_fail("rindex should not have "
					    "found the character");
				}
			} else if (tab[t].match == 1) {
				 if (tab[t].l_off != (off - &buf[a])) {
					fprintf(stderr, "a %d, t %d\n", a, t);
					atf_tc_fail("rindex returns wrong "
					    "offset");
				}
			} else {
				fprintf(stderr, "a %d, t %d\n", a, t);
