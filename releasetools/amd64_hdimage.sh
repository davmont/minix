...
then
       ...
       cp ${RELEASETOOLSDIR}/grub/grub-core/bootx64.efi ${EFI_DIR}/boot/efi
...
echo "qemu-system-x86_64 --enable-kvm -m 256 -hda `pwd`/${IMG}"
echo ""
echo "To boot this image on kvm:"
echo "cd ${MODDIR} && qemu-system-x86_64 --enable-kvm -m 256M -kernel kernel -append \"rootdevname=c0d0p0\" -initrd \"${mods}\" -hda `pwd`/${IMG}"
echo "To boot this image on kvm with EFI (tianocore OVMF):"
echo "qemu-system-x86_64 -L . -bios OVMF.fd -m 256M -drive file=${IMG},if=ide,format=raw"
